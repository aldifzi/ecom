...
<nav class="navbar fixed-top navbar-expand-lg navbar-dark bg-dark" style="background-color: <?= $this->Settings_model->general()["navbar_color"]; ?> !important">
  <div class="container">
    ...
    <div class="collapse navbar-collapse ml-5" id="navbarSupportedContent">
        ...
        <?php foreach ($menu->result_array() as $m) : ?>
          ...
          <?php if ($this->Settings_model->getSubMenu($m['id'])->num_rows() > 0) { ?>
            <li class="nav-item dropdown">
              <a class="nav-link text-dark dropdown-toggle" href="#" id="navbarDropdownCategories<?= $m['id'] ?>" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <?= $m['title']; ?>
              </a>
          ...
          <?php } else { ?>
            <li class="nav-item active">
              <a class="nav-link text-dark" href="<?= $newlink1; ?>"><?= $m['title']; ?></a>
            </li>
          <?php } ?>
        <?php endforeach; ?>
      </ul>
      ...
      <?php if ($this->session->userdata('login')) { ?>
        <a href="<?= base_url(); ?>cart" class="text-dark navbar-cart-inform">
    ...
    <?php if (!$this->session->userdata('login')) { ?>
      <div class="for-hidden">
        <i class="fa text-dark mr-3 icon-search-navbar fa-search"></i>
        <a href="<?= base_url(); ?>login" class="btn btn-sm btn-outline-light ml-2"><i class="fa fa-sign-in-alt"></i> Login</a>
      </div>
    <?php } else { ?>
      <div>
        <i class="fa search-icon-desktop text-dark ml-3 icon-search-navbar fa-search"></i>
        ...
        <div class="dropdown-menu dropdownPhotoProfileNavbarMobile" aria-labelledby="dropdownPhotoProfileNavbarMobile">
          ...
          <?php } ?>
          <a class="dropdown-item" href="<?= base_url(); ?>profile/histories">History Transaction</a>
    ...
    <div>
      <i class="fa search-mobile text-dark mr-3 icon-search-navbar fa-search"></i>
      <i class="fa fa-bars text-dark btn-for-dropdown"></i>
    </div>
...
<div class="dropdown-mobile-menu" style="background-color: <?= $this->Settings_model->general()["navbar_color"]; ?> !important">
  <?php foreach ($menu->result_array() as $m) : ?>
    <?php if ($this->Settings_model->getSubMenu($m['id'])->num_rows() > 0) { ?>
      <div class="dropdown">
        <a class="dropdown-toggle text-dark" id="dropdownMenuButton<?= $m['id'] ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          <?= $m['title']; ?></a>
    ...
    <?php } else { ?>
      ...
      ?>
      <a href="<?= $newlink1 ?>" class="text-dark"><?= $m['title']; ?></a>
    <?php } ?>
  <?php endforeach; ?>
  <div class="mt-3"></div>
  <?php if ($this->session->userdata('login')) { ?>
    <a href="<?= base_url(); ?>profile" class="text-dark"><i class="fa fa-user"></i> Profile</a>
  ...
  <?php } else { ?>
    ...
    <a href="<?= base_url(); ?>register" class="text-light"><i class="fa fa-user"></i> Register</a>
  <?php } ?>
</div>
